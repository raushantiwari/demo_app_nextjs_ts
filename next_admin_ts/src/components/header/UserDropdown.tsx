...
import { removeAuthCookie } from '@/actions/logout';
import { basicUserService } from '@/services/memberService';
import { BasicUserType } from '@/types/member.type';
...
export default function UserDropdown() {
  ...
  const userData = useCallback(async () => {
    const dataInfo = await basicUserService();
    if (
      dataInfo &&
      dataInfo?.status === 200 &&
      Object.keys(dataInfo?.data).length > 0
    ) {
      setUserInfo(dataInfo?.data);
    }
  }, []);
